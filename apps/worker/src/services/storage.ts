...
export async function downloadObject(storageKey: string): Promise<Buffer> {
  ...
  console.log(`[S3] 下载完成: ${storageKey.split('/').pop()} - ${(buffer.length / 1024).toFixed(2)}KB - 请求: ${requestTime}ms, 流: ${streamTime}ms, 总: ${totalTime}ms`);
...
export async function uploadObject(input: {
  ...
  console.log(`[S3] 上传完成: ${input.storageKey.split('/').pop()} - ${(input.body.length / 1024).toFixed(2)}KB - 上传: ${uploadTime}ms, 总: ${totalTime}ms, 速度: ${speedKBps.toFixed(2)}KB/s`);
