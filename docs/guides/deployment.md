...
本项目采用 Monorepo 结构，包含两个应用：

- **apps/web** - Next.js 前端（PWA）
- **apps/api** - Hono API 后端（内置 worker 轮询，通过 `START_WORKER=true` 开启）
...
| **Railway 部署**          | Railway    | 传统 Node.js，配置灵活   | $5/月 额度    | ⭐⭐⭐     |
...
| ------------------ | ------------------ | ---------------------- |
| Web 前端           | Vercel             | 免费                   |
| API（内置 worker） | Cloudflare Workers | 免费（100K 请求/天）\* |
| 数据库             | Neon               | 免费                   |
| 对象存储           | Cloudflare R2      | 免费                   |
| **总计**           |                    | **$0/月** 💰           |

> \* Cloudflare Workers 适合无长轮询/长连接场景。若需要常驻轮询后台任务，请改用 Railway/Render 等长驻 Node 环境。
...
| -------------------- | ------------- | ---------------------------- |
| Web 前端             | Vercel        | 免费                         |
| API（含内置 worker） | Railway       | 免费（$5 额度，约 500 小时） |
| 数据库               | Supabase      | 免费                         |
| 对象存储             | Cloudflare R2 | 免费                         |
| **总计**             |               | **$0/月** 💰                 |
