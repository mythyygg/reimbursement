...
**目标**
- 在手机端完成高频录入、票据收纳、OCR、匹配、批次检查与导出。
...
- 对象存储：S3 兼容。
- OCR：前端 OCR 优先，失败/低置信后回落云 OCR（供应商待定）。
- 导出文件保留 3 天，超期自动清理。
- 下载/导出记录必须保存。
...
### 3.3 OCR 策略
- 前端 OCR 成功即写入 `ocr_*`，可手动编辑。
- 失败或低置信：触发云 OCR（供应商可配置优先级）。
- OCR 结果只用于预填与匹配建议，不做真伪校验。
...
- OCR Worker：拉取待识别票据 → 写入 ocr 字段 → 触发候选刷新。
...
### 8.2 环境配置
- `DATABASE_URL` / `REDIS_URL` / `S3_*` / `OCR_*` / `JWT_*`
- 供应商配置支持热切换与权重优先级。
...
- 指标：OCR 成功率、匹配成功率、导出成功率、上传失败率。
- 日志：OCR 失败原因、导出失败原因、下载异常。
...
2. **增强（2 周）**：OCR 识别、候选匹配、ZIP/PDF 导出。
